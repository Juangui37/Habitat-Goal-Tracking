...
import { signInWithPopup, signOut, onAuthStateChanged, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendPasswordResetEmail, sendEmailVerification } from "firebase/auth";					
const APP_NAME = "Lumina";					
// ─── ADMIN + SECURITY ─────────────────────────────────────────────────────────					
const ADMIN_UID = "REPLACE_WITH_YOUR_UID"; // Find in Firebase Auth console					
const DISPOSABLE_DOMAINS = new Set(["mailinator.com","tempmail.com","guerrillamail.com",					
  "10minutemail.com","throwam.com","fakeinbox.com","trashmail.com","yopmail.com",					
  "sharklasers.com","getairmail.com","dispostable.com","maildrop.cc","spamgourmet.com",					
  "trashmail.me","tempr.email","discard.email","spamfree24.org","throwam.com",					
  "mailnull.com","spamgourmet.org","spam4.me","grr.la","guerrillamailblock.com",					
  "zzrgg.com","wegwerfemail.de","sogetthis.com","spamherelots.com","binkmail.com",					
  "spamavert.com","spambob.com","spamspot.com","tempemail.co","throwam.com",					
  "jetable.fr.nf","nospamfor.us","mailexpire.com","mailzilla.org","cool.fr.nf"]);					
// Usage plan limits (not enforced yet, just tracked)					
const PLAN_LIMITS = {					
...
// Usage tracker - increments per AI call					
async function trackAIUsage(uid, feature) {					
  if (!uid) return;					
  try {					
    ...
    await setDoc(ref, {					
      month,					
      totalCalls: (window._usageCount = (window._usageCount||0) + 1),					
...
function OnboardingModal({ user, onComplete, onSaveGoal, onSaveHabits }) {					
  ...
  const generateGoal = async () => {					
    ...
    setGoalLoading(true); setGoalError(""); setGeneratedGoal(null);					
    try {					
      ...
      setGoalError(`Could not generate goal: ${e.message}. Try rephrasing your idea.`);					
    }					
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,backdropFilter:"blur(12px)",padding:"16px"}}>					
      <div style={{background:T2.bg,border:"1px solid rgba(255,255,255,0.1)",borderRadius:22,width:"min(520px,100%)",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>					
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>					
          ...
          {step===2&&(					
            <div>					
              <h2 style={{fontSize:20,fontWeight:700,color:"#fff",marginBottom:6}}>What's one thing you really want to achieve?</h2>					
              <p style={{fontSize:13,color:T2.muted,marginBottom:18,lineHeight:1.5}}>Be honest — even rough ideas work. AI does the rest.</p>					
              {!generatedGoal&&(					
                <>					
                  <textarea value={goalInput} onChange={e=>setGoalInput(e.target.value)} placeholder="Describe what you want..." rows={3}					
                    style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"13px 15px",color:"#fff",fontSize:13,outline:"none",resize:"none",fontFamily:"inherit",lineHeight:1.6,boxSizing:"border-box",marginBottom:12}}/>					
                  ...
                  {goalError&&<div style={{background:"rgba(232,100,90,0.1)",border:"1px solid rgba(232,100,90,0.3)",borderRadius:10,padding:"10px 14px",color:"#E8645A",fontSize:12,marginBottom:12}}>{goalError}</div>}					
                  <button onClick={generateGoal} disabled={goalLoading||!goalInput.trim()}					
                    style={{width:"100%",background:goalInput.trim()?"linear-gradient(135deg,#9B8FE8,#7EB8D4)":"rgba(255,255,255,0.06)",border:"none",borderRadius:12,padding:"13px",color:goalInput.trim()?"#fff":"rgba(255,255,255,0.2)",cursor:goalInput.trim()?"pointer":"not-allowed",fontWeight:700,fontSize:14,fontFamily:"inherit"}}>					
                    {goalLoading?"✦ Generating your SMART goal...":"✦ Turn this into a SMART goal"}					
                  </button>					
                </>					
              )}					
        ...
        <div style={{padding:"16px 24px",borderTop:"1px solid rgba(255,255,255,0.07)",display:"flex",gap:10}}>					
          ...
          {step===2&&!generatedGoal&&<button onClick={()=>setStep(3)} style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:11,padding:"12px",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>Skip for now</button>}					
          {step===3&&(					
...
export default function App() {					
  ...
  useEffect(() => {					
    if (!user) return;					
    // Google users are pre-verified; only block email/password unverified users					
  ...
  useEffect(() => {					
    ...
    const checkOnboarding = async () => {					
      ...
      } catch(e) { /* If error, skip onboarding */ }					
  ...
  return (					
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans',system-ui,sans-serif",color:T.text,transition:"background 0.3s,color 0.3s"}}>					
      ...
      {showOnboarding&&user&&!demoMode&&(					
        <OnboardingModal					
          user={user}					
          onComplete={()=>setShowOnboarding(false)}					
          onSaveGoal={goal=>saveGoal(goal)}					
          onSaveHabits={habits=>addHabits(habits)}					
        />					
      )}					
