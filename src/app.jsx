...
        ))}	
      </div>	
      {filtered.length===0?(<div style={{textAlign:"center",padding:"60px 0"}}><div style={{fontSize:40,marginBottom:12}}>◎</div><h3 style={{color:T.muted,fontWeight:400,fontSize:14,margin:"0 0 6px"}}>No goals match</h3><p style={{color:"rgba(255,255,255,0.14)",fontSize:12,margin:0}}>Try "All" or use the AI Coach</p></div>)	
...
        </h1>	
        <p style={{fontSize:14,color:T.muted,marginBottom:40,lineHeight:1.6,maxWidth:320,margin:"0 auto 40px"}}>	
          Your goals, habits, and progress — all in one place. Synced across every device.	
        </p>	
        <button onClick={onLogin} disabled={loading}	
          style={{background:"#fff",border:"none",borderRadius:14,padding:"14px 32px",color:"#111",cursor:loading?"not-allowed":"pointer",fontWeight:700,fontSize:15,display:"inline-flex",alignItems:"center",gap:12,opacity:loading?0.7:1,fontFamily:"inherit",boxShadow:"0 4px 24px rgba(255,255,255,0.1)"}}>	
...
        </button>	
        <p style={{fontSize:11,color:"rgba(255,255,255,0.18)",marginTop:24}}>Your data is private and synced to your Google account.</p>	
      </div>	
    </div>	
  );	
}	
...
        // First login — seed goals into Firestore	
        const batch = writeBatch(db);	
        SEED_GOALS.forEach(g => batch.set(doc(db, "users", uid, "goals", g.id), g));	
        batch.commit();	
      } else {	
...
      if (snap.empty && !dataLoaded) {	
        const batch = writeBatch(db);	
        SEED_HABITS.forEach(h => batch.set(doc(db, "users", uid, "habits", h.id), h));	
        batch.commit();	
      } else {	
        const data = snap.docs.map(d => d.data());	
...
          <div style={{fontSize:10,color:T.muted,letterSpacing:3,textTransform:"uppercase",marginBottom:1}}>{greet()}, {user.displayName?.split(" ")[0]}</div>	
