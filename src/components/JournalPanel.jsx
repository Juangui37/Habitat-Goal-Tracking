...
  return (
    <div style={{background:T.card,border:"1px solid rgba(255,255,255,0.08)",borderRadius:13,padding:18,marginTop:13}}>
...
            <div style={{fontSize:10,color:T.muted,marginBottom:3}}>{j.date||j.id}</div>
            <p style={{fontSize:12,color:T.text,margin:0,lineHeight:1.5}}>{j.text||j.note}</p>
          </div>
        ))}
        {goal.journal.length===0&&<p style={{color:T.muted,fontSize:11,textAlign:"center",padding:"10px 0"}}>No entries yet</p>}
      </div>
    </div>
  );
}
